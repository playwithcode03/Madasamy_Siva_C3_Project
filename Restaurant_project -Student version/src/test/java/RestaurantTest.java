...
class RestaurantTest {Restaurant restaurant;
    ...
    public void is_restaurant_open_should_return_true_if_time_is_between_opening_and_closing_time() {
        LocalTime mockedCurrentTime = LocalTime.parse("10:31:00");
        restaurant = spy(restaurant);

        when(restaurant.getCurrentTime()).thenReturn(mockedCurrentTime);
        assertTrue(restaurant.isRestaurantOpen());
    }

    @Test
    public void is_restaurant_open_should_return_false_if_time_is_outside_opening_and_closing_time() {
        LocalTime mockedCurrentTime = LocalTime.parse("22:01:00");
        restaurant = spy(restaurant);
    ...
    //>>>>>>>>>>>>>>>>>>>>>>DISPLAY ORDER VALUE BASED ON ITEMS SELECTED <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    @Test
    public void display_order_total_value_for_selected_items(){
